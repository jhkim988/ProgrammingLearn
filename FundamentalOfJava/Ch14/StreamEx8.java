...
class Student {
  ...
  public String toString() {
    return String.format("[%s, %s, %d학년, %d학년, %d반, %3d점]", name, isMale ? "남" : "여", hak, ban, score);
  }
...
public class StreamEx8 {
  public static void main(String[] args) {
    ...
    for(List<Student> ban : stuByBan.values()) for (Student s : ban) System.out.println(s);
    Stream.of(stuArr).collect(groupingBy(Student::getBan, Collectors.toList())); // toList() 생략가능
    Stream.of(stuArr).collect(groupingBy(Student::getBan, Collectors.toCollection(HashSet::new))); // set 등 가능
    ...
    for (Student.Level key : keySet) {
      ...
      for (Student s : stuByLevel.get(key)) System.out.println(s);
      System.out.println();
    }
    ...
    Map<String, Set<Student.Level>> stuByScoreGroup =
      Stream.of(stuArr).collect(groupingBy(s -> s.getHak() + "-" + s.getBan(),
      mapping(s -> {
        if (s.getScore() >= 200) return Student.Level.HIGH;
        else if (s.getScore() >= 100) return Student.Level.MID;
        else return Student.Level.LOW;
